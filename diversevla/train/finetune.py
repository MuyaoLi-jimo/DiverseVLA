...
from OpenVLA.prismatic.vla.datasets import DiverseRLDSBatchTransform, RLDSDataset
...
class FinetuneConfig:
    # fmt: off
    vla_path: str = "/share/lmy/models/openvla-7b-finetuned-libero-object"     # Path to OpenVLA model (on HuggingFace Hub)

    # Directory Paths
    data_root_dir: Path = Path("/DATA/lmy/datasets/modified_libero_rlds")        # Path to Open-X dataset directory
    dataset_name: str = "libero_object_no_noops"                                # Name of fine-tuning dataset (e.g., `droid_wipe`)
    run_root_dir: Path = Path("runs")                               # Path to directory to store logs & checkpoints
    adapter_tmp_dir: Path = Path("./checkpoints")                     # Temporary directory for LoRA weights before fusing
    base_instruction_dir = Path("./assets")

    # Fine-tuning Parameters
    batch_size: int = 16                                            # Fine-tuning batch size
    max_steps: int = 5_000                                        # Max number of fine-tuning steps
    save_steps: int = 2_500                                          # Interval for checkpoint saving
    ...
    # model fixed
    fix_visual_encoder: bool =False                                 # fix_visual_encoder
    fix_visual_adapter: bool =False                                 # fix_visual_adapter
    fix_language_backbone: bool =False                              # fix_language_backbone
    fix_action_head: bool =False                                    # fix_action_head
    ...
    wandb_entity: str = "jimoli"                          # Name of entity to log under
...
def finetune(cfg: FinetuneConfig) -> None:
    ...
    fix_refexs = []
    if getattr(cfg, 'fix_visual_encoder', False):
        fix_refexs.append(r"vision_backbone.*")
    if getattr(cfg, 'fix_visual_adapter', False):
        fix_refexs.append(r"projector.*")
    if getattr(cfg, 'fix_language_backbone', False):
        fix_refexs.append(r"language_model\.model\.layers*")
    if getattr(cfg, 'fix_action_head', False):
        fix_refexs.append(r"language_model.model.norm.weight")
        fix_refexs.append(r"language_model.lm_head.weight")
    ...
    # )
    # ---
    replace_map = get_restruct_instructions(dataset_name=cfg.dataset_name,base_instruction_dir=cfg.base_instruction_dir)
    
    batch_transform = DiverseRLDSBatchTransform(
        action_tokenizer,
        processor.tokenizer,
        image_transform=processor.image_processor.apply_transform,
        prompt_builder_fn=PurePromptBuilder if "v01" not in cfg.vla_path else VicunaV15ChatPromptBuilder,
